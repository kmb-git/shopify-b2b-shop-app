...
const productListController = {
  ...
  createProductList: async (req, res) => {
    try {
      ...
      res.status(201).json({ message: 'ProductList created successfully!', data: savedProductList });
    } catch (error) {
      res.status(500).json({ message: 'Error creating ProductList', error: error.message });
    }
  },
  ...
  getAllProductLists: async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Error fetching ProductLists', error: error.message });
    }
  },
  ...
  getProductListById: async (req, res) => {
    try {
      const { id } = req.params;
      const productList = await ProductList.findById(id).populate('price_role_id'); // Find by ID and populate related PriceRole
      if (!productList) {
        return res.status(404).json({ message: 'ProductList not found' });
      }
      res.status(200).json(productList);
    } catch (error) {
      res.status(500).json({ message: 'Error fetching ProductList', error: error.message });
    }
  },
  ...
  updateProductList: async (req, res) => {
    try {
      ...
      const updatedProductList = await ProductList.findByIdAndUpdate(id, updates, {
      ...
      res.status(200).json({ message: 'ProductList updated successfully!', data: updatedProductList });
    } catch (error) {
      res.status(500).json({ message: 'Error updating ProductList', error: error.message });
    }
  },
  ...
  deleteProductList: async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Error deleting ProductList', error: error.message });
    }
  },
};
